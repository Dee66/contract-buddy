...
def visualize_benchmarks(results_path="data/clean/hyperparam_sweep_results.json"):
    ...
    if not filtered:
        logging.error("No valid entries with numeric 'top1', 'timing_seconds', and 'estimated_cost_usd' found for visualization.")
        sys.exit(1)
    ...
    scatter1 = sns.scatterplot(x=cost, y=top1, hue=mrr, palette="viridis", s=100, legend="brief")
    ...
    plt.figure(figsize=(10, 6))
    scatter2 = sns.scatterplot(x=cost, y=runtime, hue=top1, palette="coolwarm", s=100, legend="brief")
    for i, label in enumerate(labels):
    ...
    plt.figure(figsize=(10, 6))
    scatter3 = sns.scatterplot(x=runtime, y=top1, hue=cost, palette="plasma", s=100, legend="brief")
    for i, label in enumerate(labels):
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Plots\n")
        f.write(f"![Top-1 vs Cost](benchmark_top1_vs_cost.png)\n\n")
        f.write(f"![Runtime vs Cost](benchmark_runtime_vs_cost.png)\n\n")
        f.write(f"![Top-1 vs Runtime (colored by Cost)](benchmark_top1_vs_runtime_cost.png)\n\n")
