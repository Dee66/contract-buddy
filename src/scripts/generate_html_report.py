...
def main():
    ...
    config = get_config()
    output_path = config.get("dashboard", {}).get("output_path", "data/clean/benchmark_interactive.html")
    ...
    # Main scatter plot
    fig = px.scatter(
        df, x="cost", y="top1", color="mrr", size="runtime",
        hover_data=["r", "alpha", "dropout", "batch_size", "runtime", "cost"],
        title="Top-1 Accuracy vs Cost (interactive)",
        labels={"cost": "Estimated Cost (USD)", "top1": "Top-1 Accuracy", "mrr": "MRR", "runtime": "Runtime (s)"}
    )

    # Add a summary table of top 5 runs by Top-1
    top5 = df.sort_values("top1", ascending=False).head(5)
    table = go.Figure(data=[go.Table(
        header=dict(values=list(top5.columns), fill_color='paleturquoise', align='left'),
        cells=dict(values=[top5[c] for c in top5.columns], fill_color='lavender', align='left'))
    ])
